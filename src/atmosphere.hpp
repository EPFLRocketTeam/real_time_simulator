...
{
public:
    WhiteNoiseGen(unsigned long random_generator_seed) : generator{random_generator_seed} {
        distribution = std::normal_distribution<double>(0, 1); 
        std::cout << "WNG" << random_generator_seed << std::endl;
...
{
public:
    void setup(double dt_, double windFrom_deg, double mean_windspeed, unsigned long random_generator_seed)
    {
        std::cout << "SETUP" << random_generator_seed << std::endl;
        this->randGen = random_gen::WhiteNoiseGen(random_generator_seed);
        dt = dt_;
...
//    random_gen::UniformRandGen randGen;
    random_gen::WhiteNoiseGen randGen{0};
